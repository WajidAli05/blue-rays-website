...
function verifySuperadmin() {
  console.log("Authenticate Superadmin to change admin password\n");
  ...
  ask("Superadmin email: ")
    ...
    .then(() => ask("Admin email to update: "))
    .then((adminEmail) =>
      ask("New password: ").then((newPassword) => ({ adminEmail, newPassword }))
    )
    .then(({ adminEmail, newPassword }) =>
      Admin.findOne({ email: adminEmail, role: "admin" }).then((admin) => {
